...
const { sendEmailHc } = require('../services/send.email');
const path = require('path');
const pdf = require('html-pdf');

routes.get('/consultas-agendadas', isLoggedIn, async(request, response) => {
    if (request.user.rol == 'salud') {
        const consultas = await pool.query(`SELECT * FROM pacientes JOIN consultas ON consultas.correoPaciente = pacientes.email WHERE consultas.estado = 'agendada' AND consultas.correoDoctor='${request.user.email}' ORDER BY fechaConsulta, horaConsulta ASC`);
        ...
        const consultas = await pool.query(`SELECT * FROM doctores JOIN consultas ON consultas.correoDoctor = doctores.email WHERE consultas.estado = 'agendada' AND consultas.correoPaciente='${request.user.email}' ORDER BY fechaConsulta, horaConsulta ASC`);
        if (consultas.length > 0) {
            response.send(consultas);
        } else {
            response.send('{}');
...
routes.post('/terminar-consulta', isLoggedIn, async(request, response) => {
    if (request.user.rol == 'salud') {
        const { id, email } = request.body;
        const hcFiles = await pool.query(`SELECT * FROM hcFiles WHERE id_consulta='${id}'`);
        const formulaFiles = await pool.query(`SELECT * FROM formulaFiles WHERE id_consulta='${id}'`);
        const ordenarFiles = await pool.query(`SELECT * FROM ordenesFiles WHERE id_consulta='${id}'`);
        if (hcFiles.length > 0 && formulaFiles.length > 0) {
            if(ordenarFiles.length){
                await sendEmailHc(email, hcFiles[0].file, formulaFiles[0].file, ordenarFiles[0].file);
            }else {
                await sendEmailHc(email, hcFiles[0].file, formulaFiles[0].file, '');
            }
        } else {
            await sendEmailHc(email, hcFiles[0].file, '');
        }
        await pool.query(`UPDATE consultas SET estado='finalizada' WHERE id='${id}'`);
        const estado = {
            ok: 'ok'
        }
...
routes.post('/crear-consentimiento', isLoggedIn, async(request, response) => {
    const {
        id,
        nombre,
        apellido,
    ...
    const verificar = await pool.query(`SELECT * FROM consentimiento WHERE id_consulta='${id}' AND estado='aceptado'`);
    if (verificar.length > 0) {
        ...
        const misDatos = await pool.query(`SELECT * FROM pacientes WHERE email='${request.user.email}'`);
        const content = `<!DOCTYPE html>
        <html lang="en">
        <head>
            <meta charset="UTF-8">
            <meta name="viewport" content="width=device-width, initial-scale=1.0">
            <title>Ubimed | Consentimiento</title>
        ...
        <body style="font-size: .6em;">
            <h1 style="text-align: center; margin: 10px auto;">Consentimiento Informado</h1>
            <br>
            <br>
            <small>
                Fecha: Día ${new Date().getDate()} Mes ${new Date().getMonth()} Año ${new Date().getFullYear()}    
            </small>
            <br>
            <br>
            <p style="text-align: justify;">
                Yo, ${misDatos[0].nombre} ${misDatos[0].apellido}, Identificado con ${misDatos[0].tipoDoc} No, ${misDatos[0].cedula}, en 
                pleno uso de mis facultades mentales, otorgo de forma libre  mi consentimiento al profesiónal en ${profesion} y 
                con especialización en ${especialidad}, nombre ${nombre} ${apellido}, con numero de tarjeta profesional: ${numeroCarne} para realizar la 
        ...
        integridad de este documento.
            ...
            <div>
                <p style="margin: 0px; font-size: 1.2em;">
                    ${misDatos[0].nombre} ${misDatos[0].apellido}
                </p>
                <p style="margin: 0px; font-size: 1em;">
                    ${misDatos[0].tipoDoc}: ${misDatos[0].cedula}
                </p>
            </div>
            <br>
            <p style="text-decoration: underline">
                Firmado digitalmente:  ${misDatos[0].nombre} ${misDatos[0].apellido}
            </p>
            <br>
            <br>
            <br>
            ...
            <p style="text-align: justify">
            Que en el marco de la atención no presencial serán almacenados sus datos personales con el fin de garantizar la oportunidad, pertinencia y calidad de la atención en salud brindada. 
            </p>
            <p style="text-align: justify">
                En atención a que en el marco de la atención brindada se pueden recoger datos sensibles por tratarse de datos relativos a la salud, puede abstenerse de brindar la información solicitada.             
            </p>
            <p style="text-align: justify">
                Como responsable de la información personal de los usuarios, se tratarán los datos personales conforme a las finalidades autorizadas y según lo señalado en la política de tratamiento de datos personales, que se encuentra disponible en el siguiente enlace: www.clinicaoftalmologicaunigarro.com. 
            </p>
            <p style="text-align: justify">
            Así mismo manifiesto que me han sido explicados los derechos que me asisten como titular de los datos personales, en especial los relacionados con conocer, actualizar y rectificar los datos personales, solicitar prueba de la autorización otorgada para su tratamiento, ser informado, previa solicitud, del uso que se les ha dado a los datos, revocar la autorización y los demás derechos consagrados en el artículo 8 de la ley 1581 de 2012.
            </p>
            <p style="text-align: justify">
            Así mismo, se han implementado mecanismos para asegurar la confidencialidad y reserva de la historia clínica de conformidad con lo dispuesto en la Resolución 1995 de 1999 y normas concordantes, y en general para el tratamiento de datos de salud considerados como información sensible.
            Finalmente, manifiesto que otorgó autorización al Dr. ${nombre} ${apellido} para el tratamiento de mis datos personales en el marco de las finalidades previstas en la presente autorización. 
            El responsable de los datos personales será la Clínica Oftalmológica Unigarro domiciliado en la ciudad de Pasto(N). El correo electrónico es clinicaoftalunigarro@hotmail.com y el número telefónico de contacto es (+57)(2)7244427            
            </p>
            <br>
            <div>
                <p style="margin: 0px; font-size: 1.2em;">
        ...
        const namePdf = `consentimiento-${misDatos[0].cedula}-${id}.pdf`;
        const routeSave = path.join(__dirname, `../src/public/docs/${namePdf}`);
        pdf.create(content, {
            "border": {
            ...
            if (err) {
                ...
                const dataFile = {
                    id_consulta: id,
                    doctor: correoDoctor,
                    paciente: request.user.email,
                    rutaArchivo: routeSave,
                    nombreArchivo: namePdf
                }
                subirArchivoBD = await pool.query('INSERT INTO consentimiento SET ?', [dataFile]);
                const enviarRuta = {
                    name: namePdf
                }
                response.send(enviarRuta);
            }
        });
    }
});

routes.get('/aceptar-con/:id', isLoggedIn, async(request, response) => {
    await pool.query(`UPDATE consentimiento SET estado='aceptado' WHERE id_consulta='${request.params.id}' AND paciente='${request.user.email}'`);
    const estado = {
        accept: false
    }
    response.send(estado);
